...
class LLMClient:
    ...
    def ask_visual_subagent(self, screenshot_path: str, task_description: str, history: str, save_prompt_path: str = None) -> Tuple[Dict[str, Any], Dict[str, int]]:
        """
        封装视觉感知请求
        ...
        fallback_perception = {
            ...
            "issues": "JSON 解析错误",
            "is_completed": False,
            "use_collect_info": False
        }
        
        return self._parse_json(res_content, fallback_perception), usage_dict

    def ask_main_agent(self, task: str, step: int, max_steps: int, perception: dict, local_html: str, history: str, save_prompt_path: str = None, collected_context: str = "") -> Tuple[str, Dict[str, int]]:
        """
        封装主决策请求
    ...
    def extract_from_screenshot(self, screenshot_path: str, task: str) -> List[Dict[str, Any]]:
        """
        从磁盘截图文件提取结构化数据（每条为一个 dict）
        """
        with open(screenshot_path, "rb") as f:
            return self.extract_from_screenshot_bytes(f.read(), task)

    def extract_from_screenshot_bytes(self, jpeg_bytes: bytes, task: str) -> List[Dict[str, Any]]:
        """
        从 JPEG 二进制截图提取结构化数据，不落盘
        """
        b64 = base64.b64encode(jpeg_bytes).decode("utf-8")
        prompt = COLLECT_EXTRACT_PROMPT.format(task=task)
        messages = [{"role": "user", "content": [
            {"type": "text", "text": prompt},
            {"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{b64}"}}
        ]}]
        res, _ = self._call_llm(messages=messages, response_format={"type": "json_object"})
        parsed = self._parse_json(res, {"items": []})
        return self._coerce_collect_items(parsed)

    @staticmethod
    def _coerce_collect_items(parsed: Any) -> List[Dict[str, Any]]:
        """将模型输出统一为 dict 列表（兼容旧版直接返回数组或嵌套在对象中）。"""
        if isinstance(parsed, list):
            return [x for x in parsed if isinstance(x, dict)]
        if isinstance(parsed, dict):
            for key in ("items", "data", "results", "comments", "records"):
                v = parsed.get(key)
                if isinstance(v, list):
                    return [x for x in v if isinstance(x, dict)]
            # 单对象兜底：除常见元数据外若只剩一条业务对象
            if len(parsed) == 1:
    ...
    def _call_llm(self, messages: List[Dict[str, Any]], response_format: Optional[Dict] = None, save_prompt_path: Optional[str] = None) -> Tuple[str, Dict[str, int]]:
        ...
        self.total_input_tokens += usage.prompt_tokens if usage.prompt_tokens else 0
        self.total_output_tokens += usage.completion_tokens if usage.completion_tokens else 0
        
        usage_dict = {
            "input": usage.prompt_tokens if usage.prompt_tokens else 0,
            "output": usage.completion_tokens if usage.completion_tokens else 0
        }
